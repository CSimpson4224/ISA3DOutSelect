...
basep = (0.4*2 +sum(Ybin)/size(Ybin,1))/3;
if basep > 0.4
    basep = 0.4;
end
...
if size(unique(Ybin),1) > 1
    knt = fitcknn(Z,Ybin,'Prior',[0.6,0.4],'NumNeighbors',nn); %[1-basep,basep] [(1-basep)*0.6,basep*0.4] 1-basep,basep
    prt = predict(knt,Z);
...
if isfield(footprint,'polygon') && ~isempty(footprint.polygon.Points) && ~(footprint.polygon.Alpha==Inf) && D(2) == 2
    ...
    ii = 1;
    while footprint.purity < 0.6 && ii < size(AS,2)
