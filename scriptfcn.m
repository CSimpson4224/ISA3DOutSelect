...
ubound = max(Z); %ceil(max(Z))
lbound = min(Z); %floor(min(Z))
handle = scatter(Z(:,1), Z(:,2), 8, X, 'filled');
caxis([0,1])
xlabel('z_{1}'); ylabel('z_{2}'); title(titlelabel);
set(findall(gcf,'-property','FontSize'),'FontSize',12);
set(findall(gcf,'-property','LineWidth'),'LineWidth',1);
axis square; axis([lbound(1)-0.1 ubound(1)+0.1 lbound(2)-0.1 ubound(2)+0.1]);
...
set(findall(gcf,'-property','FontSize'),'FontSize',12);
set(findall(gcf,'-property','LineWidth'),'LineWidth',1);
axis square; axis([lbound(1)-0.3 ubound(1)+0.3 lbound(2)-0.3 ubound(2)+0.3]);
...
% =========================================================================
function h = drawGoodBadFootprint(Z, good, good2, Ybin, titlelabel)
...
if any(Ybin)
    ...
    drawFootprint(good, blue, 0.4);
    drawFootprint(good2, red, 0.4);
end
